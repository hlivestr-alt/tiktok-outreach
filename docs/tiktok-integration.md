...
An observed-saturated bounded specific-GMV seed at least 500 followers wide receives one inclusive midpoint exploratory split regardless of historical yield. Production follower children start with null search and page cursors and inherit the parent’s exact GMV bucket and range. Deeper follower recursion waits for both siblings, stops the branch when combined incremental yield is below 5%, and otherwise requires each child to remain observed-saturated with at least 10% incremental yield. These are follower-yield signals; no unique-creator-rate stop rule is added. Minimum child widths are configurable by follower region: 50, 100, 1,000, 10,000, and 100,000. Follower splitting remains inside the selected GMV branch; adaptive leaves never create new GMV-All or on-demand GMV children. The open-ended 5,000,000+ seed never receives an invented upper bound and still carries its selected official GMV bucket.

Claiming is PostgreSQL-backed and deterministic. The persisted claim sequence drives a seven-claim cycle with five HIGH slots, one MEDIUM slot, and one FIFO V2 EXPLORATION slot, for a 14.2857% seed target while all three pools have work. Pool fallbacks are deterministic, so work continues if a class is temporarily empty. The FIFO exploration slot guarantees every untouched seed eventually advances, HIGH receives five of seven opportunities, and MEDIUM receives one of seven; exploration rotates its target GMV bucket deterministically so untouched buckets receive fair opportunities. Category and follower evidence is filtered within the candidate’s exact GMV branch, so one bucket cannot borrow evidence from another. The already-active partition and cursor always resume before the selector runs. A claimed partition runs to natural provider termination.
...
V1 creators, historical V2 GMV-All seeds, new specific-GMV seeds, experiment rows, partition statuses, receipts, and the legacy `all-creators` row remain historical. Scheduler migrations are additive, never reactivate `EXPERIMENT` rows or unfinished GMV-All work, preserve the exact active continuation and opaque cursor fields, and leave the global job paused until an operator resumes it. Per-partition request, throttle, returned-row, new-global-Open-ID, duplicate, yield, saturation, classification, depth, scheduler decision, and priority metrics stay in PostgreSQL and never enter the creator Sheet.
