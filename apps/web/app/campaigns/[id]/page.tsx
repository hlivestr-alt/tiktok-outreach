...
export default function CampaignPage() {
  ...
  return <div className="page"><Link className="back-link" href="/campaigns"><ArrowLeft size={16}/>Campaigns</Link>
    ...
    {campaign.state === "PREVIEW_READY" && <section className="confirmation-card"><div><h2>{readOnly ? "READ_ONLY_PREVIEW is ready" : "Preview is ready"}</h2><p>{readOnly ? summary.historyIdentityCoverageIncomplete ? "Real discovery is available for analysis, with incomplete history identity coverage clearly flagged. Real dispatch is unavailable." : "Real discovery and linked-history checks are complete. Real dispatch is unavailable." : `Freeze ${formatNumber(selectedCount)} selected creators. Reservations last 30 minutes.`}</p></div>{!readOnly && <button className="button primary" onClick={() => action("freeze", { version: campaign.version })}><Check size={17}/>Freeze recipients</button>}</section>}
