...
export default function CampaignPage() {
  ...
  const sendReason = eligibleCount <= 0 || selectedCount <= 0
    ? "No eligible recipients are available to send"
    : outboundUnavailable ? campaign.outboundCapability?.reason ?? "Outbound capability is unavailable" : null;
  const sendDisabled = sending || eligibleCount <= 0 || selectedCount <= 0 || outboundUnavailable;
  ...
  return <div className="page"><Link className="back-link" href="/campaigns"><ArrowLeft size={16}/>Campaigns</Link>
    ...
    {campaign.progress.frozen > 0 && <><section className="metric-strip"><div><span>Frozen</span><strong>{formatNumber(campaign.progress.frozen)}</strong></div><div className="accent"><span>Completed</span><strong>{formatNumber(campaign.progress.completed)}</strong></div><div><span>Remaining</span><strong>{formatNumber(campaign.progress.remaining)}</strong></div><div><span>Sent</span><strong>{formatNumber(deliveryCounts.SENT)}</strong></div><div><span>Restricted</span><strong>{formatNumber(deliveryCounts.RESTRICTED)}</strong></div><div><span>Failed / unknown</span><strong>{formatNumber((deliveryCounts.FAILED ?? 0) + (deliveryCounts.DELIVERY_UNKNOWN ?? 0) + (deliveryCounts.DELIVERY_UNKNOWN_UNRESOLVED ?? 0))}</strong></div></section><div className="alert neutral"><Clock/><div><strong>Production pacing: maximum 1 recipient message per second</strong><span>Ideal minimum at this campaign size is approximately {idealDuration}, before provider cooldowns, retries, restrictions, failures, or downtime.</span></div></div></>}
